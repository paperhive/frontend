        ...
        link: function (scope, elm, attrs, ctrl) {
          ctrl.$asyncValidators.username = function (modelValue, viewValue) {
            ...
            if (ctrl.$isEmpty(modelValue) ||
                (authService.user && authService.user.username === modelValue)) {
              return $q.when();
            }
            var defer = $q.defer();

            $http.head(config.api_url + '/users/byUsername/' + modelValue)
              ...
              .error(function (data, status) {
                ...
                defer.reject('An error occured while checking if the username is available');
              });
